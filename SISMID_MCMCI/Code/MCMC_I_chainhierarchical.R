...
chain_hierarchical_reduced = function(mcmc.size){
  ...
  for (i in 2:mcmc.size){
    ...
    for (j in 1:n1){
      q[i,j] = rbeta(1, 2+cur.alpha, cur.beta)   # YOUR TASK IS TO COMPLETE THIS. USE THE 'rbeta' FUNCTION
    }
    
    # households with chain 1->1
    for (j in 1:n11){
      q[i,j+n1] = rbeta(1, 2+cur.alpha, 1+cur.beta) # YOUR TASK IS TO COMPLETE THIS. USE THE 'rbeta' FUNCTION
    }
    ...
    for (j in 1:275){
      q[i,j+n1+n11] = rbeta(1, n111[i-1,j]+cur.alpha, 2+cur.beta) # YOUR TASK IS TO COMPLETE THIS. USE THE 'rbeta' FUNCTION
    }
...
abline(v=mean(mcmc.al), col=2)
hist(mcmc.be,xlab='beta',main='')
abline(v=mean(mcmc.be), col=2)

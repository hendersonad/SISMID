...
###################################################################
  ...
  #call ode-solver lsoda to integrate ODEs with strong IFN effect
  e=0.9; r=0.0;
  odeoutput4=lsoda(Y0,timevec,odeequations,c(parms,e,r));
  e=0.9; r=0.5;
  odeoutput5=lsoda(Y0,timevec,odeequations,c(parms,e,r));
  ...
  lines(odeoutput4[,1],odeoutput4[,3]+odeoutput4[,4],type="l",col=4,lwd=2, lty=2)
  lines(odeoutput5[,1],odeoutput5[,3]+odeoutput5[,4],type="l",col=2,lwd=2, lty=2)
  legend(1,1e3, c("no drug","IFN","IFN + rib","strong IFN only", "Strong IFN + rib"),col = c("green","blue","red", 4,2),lwd=2, lty=c(1,1,1,2,2))
